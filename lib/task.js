...
module.exports = class Task {
    ...
    async build(parameters = {}) {
        ...
        if (ctx.needInstallNPM()) {
            if(await this.installNPM(ctx)) {
                console.dir('FUCK!');
                parameters.forceRebuildImages = true; // todo: man, this is way terrible even for you!
            }
        }
	    ...
	    return new Promise((resolve, reject) => {
            ...
            if (this.needDumpBundleStats()) {
                const folder = ctx.getTemporaryFolder('stats');
                this.getApplication().getProject().debug(`Writing stats to ${folder}`);
                return tool.saveStats(stats, folder);
            }
        }).then(() => {
            const onAfterBuild = this.getOnAfterBuild();
            if (_.isFunction(onAfterBuild)) {
                return onAfterBuild(ctx);
            }
        });
    }
    ...
    needDumpBundleStats() {
        return this.getIncludes().getParameters().dumpBundleStats === true;
    }
